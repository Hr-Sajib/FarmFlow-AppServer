...
const createPostValidationSchema = z.object({
  body: z.object({
    creatorName: z.string({ invalid_type_error: "Creator name must be a string", required_error: "Creator name is required" }).trim().min(1, "Creator name cannot be empty"),
    creatorId: z.string({ invalid_type_error: "Creator ID must be a string", required_error: "Creator ID is required" }).refine(
      (value) => /^[0-9a-fA-F]{24}$/.test(value),
    ...
    postImages: z.array(z.string({ invalid_type_error: "Post image must be a string" }).url({ message: "Post image must be a valid URL" }).optional()).optional(),
    reactions: reactionSchema.optional(),
    comments: z.array(commentSchema).optional(),
    postTopic: z.array(postTopicEnum, { invalid_type_error: "Post topics must be an array", required_error: "At least one post topic is required" }).min(1, "At least one post topic is required"),
  }).strict(),
});
